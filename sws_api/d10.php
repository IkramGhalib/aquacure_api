...
//   AND date(datetime) < curdate()LIMIT 7";
  $query="SELECT FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(datetime) / (60*60)) * (60*60)) AS hourlytime, round(avg(D_10)) as D_10 from devices_logs where eid='$device_id' GROUP BY hourlytime ORDER BY hourlytime desc limit 7";
  $result=mysqli_query($conn,$query);
  $response=array();
  if($result){
